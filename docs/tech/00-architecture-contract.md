...
- Multiplayer: **both halves are written; nothing is deployed.** `net/MatchTransport` is the only way the battle screen reaches a match, and `LocalTransport` and `WsTransport` are two implementations of it. Per-seat event redaction, the §5.2 view sanitization, the view reducer and the wire schemas all run in the **offline** build, so a hidden-information leak surfaces in tests instead of in production against a real opponent — which it did, repeatedly (see `docs/tech/03-multiplayer-architecture.md` §5.1.1, §5.1.2, §5.3, §7.7, §2.1). `server/` holds the authoritative room, the gateway and the casual queue; `tests/transport-conformance.test.ts` runs one script against both transports so the interface claim is checked rather than asserted.
- **"Written" is not "shipped", and the gate is deployment.** No Cloudflare account exists, `SUPABASE_URL` is empty, and with it empty every token is refused — so the queue is unreachable and an online tile that appeared to work would be a lie. No fake online UI: modes that need a server are shown as "coming online", **greyed but still focusable**, each with a one-paragraph explainer of the designed feature (`design/03-screens-and-navigation.md`). A bare `disabled` button reading "needs server" is explicitly the wrong answer — it reads as a fault, and it puts the explanation out of reach of keyboard and screen-reader users. Enforced by `tests/mode-honesty.test.ts`.
